...
object FluentExamples extends App {
  ...
  private def getController(
      listRetrieval: Future[ReplicationControllerList],
      rcName: String
  ): Future[ReplicationController] =
    listRetrieval map { list =>
  ...
  def buildNginxControllers: Future[List[ReplicationController]] = {
    ...
    Future.sequence(
      List(
        devController,
        internalTestController,
        externalTestController,
        internalProdController,
        externalProdController
      )
    )
  }
  ...
  def deployControllers: Future[List[ReplicationController]] = {
    ...
    listCurrentControllers flatMap { existingRCList =>
      ...
      newControllerBuilds flatMap { rcDeploymentList =>
        ...
        val doUpdates = updateList map { rc =>
          k8s update rc
        }
        val doCreates = createList map { rc =>
          k8s create rc
        }
  ...
  def deployServices: Future[List[Service]] = {
    ...
    listCurrentServices flatMap { existingSvcList =>
      ...
      newServiceBuilds flatMap { svcDeploymentList =>
        ...
        val doUpdates = updateList map { svc =>
          k8s update svc
        }
        val doCreates = createList map { svc =>
          k8s create svc
        }
  ...
  def deployNginxServices = {
    ...
    deployAll.failed.foreach {
      case ex: K8SException => System.err.println("Request failed with status : " + ex.status)
    }
