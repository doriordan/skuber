...
object KubernetesProxyActor {
  ...
  case class CreateReplicationController(rcSpec: ReplicationController, resultHandler: ActorRef)
      extends KubernetesRequestMessage
  case class GetReplicationController(name: String, resultHandler: ActorRef) extends KubernetesRequestMessage
  case class UpdateReplicationController(newSpec: ReplicationController, resultHandler: ActorRef)
      extends KubernetesRequestMessage
...
class KubernetesProxyActor extends Actor with ActorLogging {
  ...
  def receive = LoggingReceive {
    ...
    case DeleteReplicationController(name, resultHandler) =>
      invoke(k8s delete [ReplicationController] name) pipeTo resultHandler
    case CreateService(serviceSpec, resultHandler) => invoke(k8s create [Service] serviceSpec) pipeTo resultHandler
    case CreateReplicationController(rcSpec, resultHandler) =>
      invoke(k8s create [ReplicationController] rcSpec) pipeTo resultHandler
    case GetReplicationController(name: String, resultHandler) =>
      invoke(k8s get [ReplicationController] name) pipeTo resultHandler
    case UpdateReplicationController(newSpec, resultHandler) =>
      invoke(k8s update [ReplicationController] newSpec) pipeTo resultHandler
