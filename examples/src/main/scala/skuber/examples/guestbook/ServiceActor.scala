...
class ScaleResultHandler(consumer: ActorRef, name: String) extends ServiceResultHandler(consumer) {
  override def receive = LoggingReceive {
    case ScalerActor.ScalingError =>
      complete(UnexpectedServiceError(name, new Exception("An error occured while scaling")))
    case akka.actor.Status.Failure(ex) => complete(UnexpectedServiceError(name, ex))
    case ResourceNotFound =>
      complete(UnexpectedServiceError(name, new Exception("Unable to scale as resource does not exist")))
