...
class ScalerActor(kubernetes: ActorRef, controllerName: String, targetReplicaCount: Int)
    extends Actor
    with ActorLogging {
  ...
  def updateSpecification: Receive =
    maybeNotFound orElse specNeedsUpdating orElse isCompleted orElse startWaiting orElse handleFailureStatus
