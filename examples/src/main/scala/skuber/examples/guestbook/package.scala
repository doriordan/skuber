...
package object model {
  ...
  case class GuestbookServiceSpecification(
    ...
    def buildKubernetesResources = {
      ...
      val template =
        Pod.Template.Spec.named(serviceName).addLabels(labels).addAnnotations(customAnnotations).addContainer(container)
      ...
      val service = Service(
        metadata = ObjectMeta(name = serviceName, labels = labels),
