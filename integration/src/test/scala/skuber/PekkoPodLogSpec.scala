...
class PekkoPodLogSpec extends PodLogSpec with PekkoK8SFixture  {
  ...
  it should "get log of a pod" in { k8s =>
    ...
    pekkoK8s.getPodLogSource(podName, LogQueryParams(follow = Some(false))).flatMap { source =>
  ...
  it should "tail log of a pod and timeout after a while" in { k8s =>
    ...
    pekkoK8s.getPodLogSource(podName, LogQueryParams(follow = Some(true))).flatMap { source =>
