...
package object format {
  ...
  implicit val subjectFormat: Format[Subject] = (
    ...
    )(Subject.apply, s => (s.apiVersion, s.kind, s.name, s.namespace))
  ...
  )(PolicyRule.apply, p => (p.apiGroups, p.attributeRestrictions, p.nonResourceURLs, p.resourceNames, p.resources, p.verbs))

  implicit val roleFormat: Format[Role] = (
    objFormat and
    (JsPath \ "rules").formatMaybeEmptyList[PolicyRule]
  )(Role.apply, r => (r.kind, r.apiVersion, r.metadata, r.rules))
  ...
  )(RoleRef.apply, r => (r.apiGroup, r.kind, r.name))
  ...
  implicit val clusterRoleBindingFormat: Format[ClusterRoleBinding] = (
    ...
    )(ClusterRoleBinding.apply, r => (r.kind, r.apiVersion, r.metadata, r.roleRef, r.subjects))

  implicit val clusterRoleFormat: Format[ClusterRole] = (
    objFormat and
      (JsPath \ "rules").formatMaybeEmptyList[PolicyRule]
    )(ClusterRole.apply, c => (c.kind, c.apiVersion, c.metadata, c.rules))

  implicit val roleBindingFormat: Format[RoleBinding] = (
    objFormat and
      (JsPath \ "roleRef").format[RoleRef] and
      (JsPath \ "subjects").formatMaybeEmptyList[Subject]
    )(RoleBinding.apply,  r => (r.kind, r.apiVersion, r.metadata, r.roleRef, r.subjects))
