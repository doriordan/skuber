...
case class ReplicationController(
    ...
    def withReplicas(n: Int) = {  this.copy(spec = Some(spec.getOrElse(new ReplicationController.Spec).copy(replicas = Some(n))))}
    def withSelector(s: Map[String, String]) = { this.copy(spec = Some(spec.getOrElse(new ReplicationController.Spec).copy(selector = Some(s)))) }    
    def withPodTemplate(t: Pod.Template.Spec) = { this.copy(spec = Some(spec.getOrElse(new ReplicationController.Spec).copy(template = Some(t)))) }  
