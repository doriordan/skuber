...
class WatchSourceSpec extends Specification with MockitoSugar {
  ...
  "WatchSource" should {
    "read event continuously with no name specified and from a point in time" >> {
      ...
      when(client.buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(Uri.Query("timeoutSeconds" -> "1", "resourceVersion" -> "12802", "watch" -> "true")), None)).thenReturn(firstRequest)
      when(client.buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(Uri.Query("timeoutSeconds" -> "1", "resourceVersion" -> "12804", "watch" -> "true")), None)).thenReturn(secondRequest)

      val responses = Map(firstRequest -> HttpResponse(StatusCodes.OK, entity = createHttpEntity(retrieveWatchJson("/watchReplicationControllerFirstRequest.json"))),
      ...
      verify(client).buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(Uri.Query("timeoutSeconds" -> "1", "resourceVersion" -> "12802",  "watch" -> "true")), None)
      verify(client).buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(Uri.Query("timeoutSeconds" -> "1", "resourceVersion" -> "12804", "watch" -> "true")), None)
      ok
    }
    ...
    "read event continuously with no name specified from the beginning" >> {
      ...
      when(client.logConfig).thenReturn(LoggingConfig())
      when(client.buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(Uri.Query("timeoutSeconds" -> "1", "watch" -> "true")), None)).thenReturn(firstRequest)
      when(client.buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(Uri.Query("timeoutSeconds" -> "1", "resourceVersion" -> "12804", "watch" -> "true")), None)).thenReturn(secondRequest)
      ...
      verify(client, times(4)).logConfig
      verify(client).buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(Uri.Query("timeoutSeconds" -> "1", "watch" -> "true")), None)
      verify(client).buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(Uri.Query("timeoutSeconds" -> "1", "resourceVersion" -> "12804", "watch" -> "true")), None)
      ok
    }
    ...
    "read event continuously with name specified from a point in time" >> {
      ...
      when(client.buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(query1), None)).thenReturn(firstRequest)
      when(client.buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(query2), None)).thenReturn(secondRequest)
      ...
      verify(client, times(4)).logConfig
      verify(client).buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(query1), None)
      verify(client).buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(query2), None)
    ...
    "handle empty responses from the cluster when request timeout times out" >> {
      ...
      when(client.logConfig).thenReturn(LoggingConfig())
      when(client.buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(Uri.Query("timeoutSeconds" -> "1", "resourceVersion" -> "12802", "watch" -> "true")), None)).thenReturn(firstRequest)
      when(client.buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(Uri.Query("timeoutSeconds" -> "1", "resourceVersion" -> "12804", "watch" -> "true")), None)).thenReturn(secondRequest).thenReturn(thirdRequest)
      ...
      verify(client).buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(Uri.Query("timeoutSeconds" -> "1", "resourceVersion" -> "12802", "watch" -> "true")), None)
      verify(client, times(2)).buildRequest(HttpMethods.GET, skuber.ReplicationController.rcDef, None, Some(Uri.Query("timeoutSeconds" -> "1", "resourceVersion" -> "12804", "watch" -> "true")), None)
      ok
    }
    ...
    "handle bad input from cluster" >> {
      ...
      val responses = Map(firstRequest -> HttpResponse(StatusCodes.OK, entity = "bad input"))
    ...
    "handle bad json from cluster" >> {
      ...
      val responses = Map(firstRequest -> HttpResponse(StatusCodes.OK, entity = createHttpEntity("{asdf:asdfa}")))
    ...
    "handle a HTTP 500 error from service" >> {
      ...
      val responses = Map(firstRequest -> HttpResponse(StatusCodes.InternalServerError))
    ...
    "handle a HTTP 401 error from service" >> {
      ...
      val responses = Map(firstRequest -> HttpResponse(StatusCodes.Unauthorized))
  ...
  private def buildWatchEvent(resourceVersion: String, replicas: Int) = {
    WatchEvent(EventType.MODIFIED,
      ReplicationController(metadata = ObjectMeta("frontend", "", "default", "246f12b6-719b-11e5-89ae-0800279dd272",
        ...
        spec = Some(ReplicationController.Spec(0, Some(Map("name" -> "frontend")), Some(Pod.Template.Spec(ObjectMeta("frontend", "", "default", "", "", "", None,
                None, None, Map("name" -> "frontend"), Map(),
                Nil, 0, None, None),
              Some(Pod.Spec(containers = List(Container("php-redis",
                      "kubernetes/example-guestbook-php-redis:v2",
                      ports = List(Container.Port(80, Protocol.TCP)),
