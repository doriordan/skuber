...
class ExecSpec extends K8SFixture with Eventually with Matchers with BeforeAndAfterAll with ScalaFutures {
  ...
  it should "execute a command that outputs to stderr in the running pod" in { k8s =>
    ...
    var errorOutput = ""
    val stderr: Sink[String, Future[Done]] = Sink.foreach(errorOutput += _)
    k8s.exec(podName3, Seq("sh", "-c", "whoami >&2"), maybeStdout = Some(stdout), maybeStderr = Some(stderr), maybeClose = Some(closeAfter(1.second))).futureValue
  ...
  it should "execute a command in an interactive shell of the running pod" in { k8s =>
    ...
    var errorOutput = ""
    val stderr: Sink[String, Future[Done]] = Sink.foreach(errorOutput += _)
    k8s.exec(podName4, Seq("sh"), maybeStdin = Some(stdin), maybeStdout = Some(stdout), maybeStderr = Some(stderr), tty = true, maybeClose = Some(closeAfter(1.second))).futureValue
  ...
  it should "throw an exception without stdin, stdout nor stderr in the running pod" in { k8s =>
    ...
    k8s.create(getNginxPod(podName5, "1.7.9")).futureValue
    Thread.sleep(5000)
    whenReady(
      k8s.exec(podName5, Seq("whoami")).failed
    ) { result =>
  ...
  it should "throw an exception against an unexisting pod" in { k8s =>
    ...
    k8s.create(getNginxPod(podName6, "1.7.9")).futureValue
    Thread.sleep(5000)
    whenReady(
      k8s.exec(podName6 + "x", Seq("whoami")).failed
    ) { result =>
