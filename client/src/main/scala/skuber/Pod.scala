...
object Pod {
  ...
  case class Spec(
    ...
    def addNodeSelector(kv: Tuple2[String, String]): Spec = {
  ...
  case object Affinity {
    ...
    case object NodeAffinity {
      ...
      case object RequiredDuringSchedulingIgnoredDuringExecution {
        ...
        def requiredQuery(
            key: String,
            operator: NodeSelectorOperator.Value,
            values: List[String]
  ...
  case class DNSConfig(
      nameservers: List[String] = Nil,
      options: List[DNSConfigOption] = Nil,
      searches: List[String] = Nil
  )
  ...
  case class Template(
    ...
    def addLabel(label: Tuple2[String, String]): Template =
    ...
    def addAnnotation(anno: Tuple2[String, String]): Template =
    ...
    def addAnnotations(newAnnos: Map[String, String]): Template =
      this.copy(metadata = metadata.copy(annotations = metadata.annotations ++ newAnnos))
  ...
  object Template {
    ...
    case class Spec(metadata: ObjectMeta = ObjectMeta(), spec: Option[Pod.Spec] = None) {

      def addLabel(label: Tuple2[String, String]): Spec =
        this.copy(metadata = metadata.copy(labels = metadata.labels + label))
      def addLabels(newLabels: Map[String, String]): Spec =
        this.copy(metadata = metadata.copy(labels = metadata.labels ++ newLabels))
      def addAnnotation(anno: Tuple2[String, String]): Spec =
        this.copy(metadata = metadata.copy(annotations = metadata.annotations + anno))
  ...
  case class EqualToleration(
      key: String,
      value: Option[String] = None,
      effect: Option[TolerationEffect] = None,
      tolerationSeconds: Option[Int] = None
  ) extends Toleration
  case class ExistsToleration(
