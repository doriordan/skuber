...
case class NetworkPolicy(
  ...
  def selectAllPods: NetworkPolicy =
    this.copy(spec = spec.map(s => s.copy(podSelector = LabelSelector())).orElse(specSelectingAllPods))
  def selectPods(podSelector: LabelSelector): NetworkPolicy =
    this.copy(
      spec =
        spec.map(s => s.copy(podSelector = podSelector)).orElse(Some(NetworkPolicy.Spec(podSelector = podSelector)))
    )
  ...
  def applyIngressPolicy: NetworkPolicy =
    fallbackToSelectingAllPods.copy(spec = spec.map(s => s.copy(policyTypes = "Ingress" :: s.policyTypes)))
  def applyEgressPolicy: NetworkPolicy =
    fallbackToSelectingAllPods.copy(spec = spec.map(s => s.copy(policyTypes = "Egress" :: s.policyTypes)))
...
object NetworkPolicy {
  ...
  )(IPBlock.apply, unlift(IPBlock.unapply))
  ...
  )(IngressRule.apply, unlift(IngressRule.unapply))

  implicit val egressRuleFmt: Format[EgressRule] = (
    (JsPath \ "ports").formatMaybeEmptyList[Port] and
      (JsPath \ "to").formatMaybeEmptyList[Peer]
  )(EgressRule.apply, unlift(EgressRule.unapply))
  ...
  )(NetworkPolicy.apply, unlift(NetworkPolicy.unapply))
}
