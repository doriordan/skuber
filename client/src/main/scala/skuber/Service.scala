...
case class Service(
  ...
  def addLabel(lbl: Tuple2[String, String]): Service =
    this.copy(metadata = metadata.copy(labels = metadata.labels + lbl))
  def addLabels(lbls: Map[String, String]): Service =
    this.copy(metadata = metadata.copy(labels = metadata.labels ++ lbls))
  ...
  def addExternalIP(ip: String): Service =
    this.copy(spec = Some(copySpec.copy(externalIPs = ip :: copySpec.externalIPs)))

  def withSessionAffinity(affinity: Service.Affinity.Value): Service =
    this.copy(spec = Some(copySpec.copy(sessionAffinity = affinity)))

  import Endpoints._
  def mapsToEndpoint(ip: String, port: Int, protocol: Protocol.Value = Protocol.TCP): Endpoints =
    Endpoints(
      metadata = ObjectMeta(name = this.name, namespace = this.ns),
      subsets = Subset(Address(ip) :: Nil, Nil, Port(port, protocol) :: Nil) :: Nil
    )
  ...
  def mapsToEndpoints(subsets: List[Subset]): Endpoints =
    Endpoints(metadata = ObjectMeta(name = this.name, namespace = this.ns), subsets = subsets)
}
