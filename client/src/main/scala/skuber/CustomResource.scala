...
case class CustomResource[Sp, St](
  ...
  def withMetadata(metadata: ObjectMeta): CustomResource[Sp, St] = this.copy(metadata = metadata)
  def withName(name: String): CustomResource[Sp, St] = withMetadata(metadata.copy(name = name))
  def withGenerateName(generateName: String): CustomResource[Sp, St] =
    withMetadata(metadata.copy(generateName = generateName))
  def withNamespace(namespace: String): CustomResource[Sp, St] = withMetadata(metadata.copy(namespace = namespace))
  def withLabels(labels: Tuple2[String, String]*): CustomResource[Sp, St] =
    withMetadata(metadata.copy(labels = Map(labels: _*)))
  def withAnnotations(annotations: Tuple2[String, String]*): CustomResource[Sp, St] =
    withMetadata(metadata.copy(annotations = Map(annotations: _*)))
  def withFinalizers(finalizers: String*): CustomResource[Sp, St] =
    withMetadata(metadata.copy(finalizers = Some(finalizers.toList)))
...
object CustomResource {
   ...
   * definition.
   */
  def apply[Sp, St](spec: Sp)(implicit rd: ResourceDefinition[CustomResource[Sp, St]]) =
    new CustomResource[Sp, St](
  ...
  /**
    ...
    */
  implicit def enableStatus[Sp, St]: HasStatusSubresource[CustomResource[Sp, St]] =
    new HasStatusSubresource[CustomResource[Sp, St]] {}
   ...
   */
  def enableScaling[O <: ObjectResource]: Scale.SubresourceSpec[O] =
  ...
  implicit def crFormat[Sp, St](implicit spFmt: Format[Sp], stFmt: Format[St]): Format[CustomResource[Sp, St]] =
    ...
    )(CustomResource.apply, unlift(CustomResource.unapply[Sp, St]))
   ...
   */
  implicit def crListFormat[CustomResource[Sp, St] <: ObjectResource, Sp, St](
      implicit ofmt: Format[CustomResource[Sp, St]]
  ): Format[ListResource[CustomResource[Sp, St]]] = {
