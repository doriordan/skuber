...
case class ReplicationController(
  ...
  def withResourceVersion(version: String): ReplicationController =
    this.copy(metadata = metadata.copy(resourceVersion = version))
  def addLabel(label: Tuple2[String, String]): ReplicationController =
  ...
  def addAnnotation(anno: Tuple2[String, String]): ReplicationController =
  ...
  def withSelector(s: Map[String, String]): ReplicationController =
    this.copy(spec = Some(copySpec.copy(selector = Some(s))))
  def withSelector(s: Tuple2[String, String]): ReplicationController = withSelector(Map(s))

  def withTemplate(t: Pod.Template.Spec): ReplicationController =
    this.copy(spec = Some(copySpec.copy(template = Some(t))))
...
object ReplicationController {
  ...
  implicit val rcListDef: ResourceDefinition[ReplicationControllerList] =
    new ResourceDefinition[ReplicationControllerList] { def spec: CoreResourceSpecification = specification }
