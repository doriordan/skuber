...
object TLS {
  ...
  val HttpsPattern: Regex = "https:.*".r
  val HttpPattern: Regex = "http:.*".r
  ...
  def establishSSLContext(k8sContext: Context): Option[SSLContext] = {
    k8sContext.cluster.server match {
      ...
      case _ =>
        throw new Exception(
          "Kubernetes cluster API server URL does not begin with either http or https : "
  ...
  private def getTrustManagers(
      ...
      serverCertConfig: Option[PathOrData]
  ): Option[Array[TrustManager]] =
    if (skipTLSVerify)
