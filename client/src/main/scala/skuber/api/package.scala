...
package object client {
  ...
  final case class CertAuth(clientCertificate: PathOrData, clientKey: PathOrData, user: Option[String])
      extends AuthInfo {
  ...
  class RequestContext(
    ...
    private[skuber] def invoke(request: HttpRequest, watch: Boolean = false)(
    ...
    private[skuber] def logInfoOpt(enabledLogEvent: Boolean, msgOpt: => Option[String])(
        implicit lc: LoggingContext
    ): Unit = {
      if (log.isInfoEnabled && enabledLogEvent) {
        msgOpt foreach { msg =>
    ...
    private[skuber] def logWarn(msg: String)(implicit lc: LoggingContext): Unit = {
      log.error(s"[ ${lc.output} - $msg ]")
    }

    private[skuber] def logError(msg: String)(implicit lc: LoggingContext): Unit = {
      log.error(s"[ ${lc.output} - $msg ]")
    }
    ...
    private[skuber] def logDebug(msg: => String)(implicit lc: LoggingContext): Unit = {
      if (log.isDebugEnabled)
        log.debug(s"[ ${lc.output} - $msg ]")
    }

    private[skuber] def logRequestObjectDetails[O <: ObjectResource](method: HttpMethod, resource: O)(
        implicit lc: LoggingContext
    ): Unit = {
      logInfoOpt(
        logConfig.logRequestBasicMetadata, {
          val name = resource.name
          val version = resource.metadata.resourceVersion
          method match {
            case HttpMethods.PUT | HttpMethods.PATCH =>
              Some(s"Requesting update of resource: { name:$name, version:$version ... }")
    ...
    private[skuber] def logReceivedObjectDetails[O <: ObjectResource](
        resource: O
    )(implicit lc: LoggingContext): Unit = {
      logInfo(
        logConfig.logResponseBasicMetadata,
    ...
    private[skuber] def logReceivedListDetails[L <: ListResource[_]](result: L)(implicit lc: LoggingContext): Unit = {
    ...
    private[skuber] def makeRequestReturningObjectResource[O <: ObjectResource](
    ...
    private[skuber] def makeRequestReturningListResource[L <: ListResource[_]](
        httpRequest: HttpRequest
    ...
    /**
      ...
      */
    private[skuber] def modify[O <: ObjectResource](
        method: HttpMethod
    )(obj: O)(implicit fmt: Format[O], rd: ResourceDefinition[O], lc: LoggingContext): Future[O] = {
    ...
    private[skuber] def modify[O <: ObjectResource](
      ...
      for {
        ...
        httpRequest = buildRequest(method, rd, nameComponent).withEntity(
          requestEntity.withContentType(MediaTypes.`application/json`)
        )
    ...
    def updateStatus[O <: ObjectResource](obj: O)(
        implicit
        fmt: Format[O],
    ...
    def getStatus[O <: ObjectResource](name: String)(
        implicit
        fmt: Format[O],
        rd: ResourceDefinition[O],
        lc: LoggingContext = RequestLoggingContext(),
    ...
    def listByNamespace[L <: ListResource[_]]()(
        implicit fmt: Format[L],
        rd: ResourceDefinition[L],
        lc: LoggingContext = RequestLoggingContext()
    ): Future[Map[String, L]] = {
      listByNamespace[L](rd)
    }

    private def listByNamespace[L <: ListResource[_]](
        rd: ResourceDefinition[_]
      ...
      val tuplesFut: Future[List[(String, L)]] = nsNamesFut flatMap { nsNames: List[String] =>
        Future.sequence(nsNames map { nsName: String =>
          listInNamespace[L](nsName, rd) map { l =>
            (nsName, l)
          }
        })
      }
     ...
     */
    def listInNamespace[L <: ListResource[_]](
        theNamespace: String
    ...
    private def listInNamespace[L <: ListResource[_]](
        theNamespace: String,
        rd: ResourceDefinition[_]
    )(implicit fmt: Format[L], lc: LoggingContext): Future[L] = {
     ...
     */
    def listSelected[L <: ListResource[_]](
        labelSelector: LabelSelector
    )(implicit fmt: Format[L], rd: ResourceDefinition[L], lc: LoggingContext = RequestLoggingContext()): Future[L] = {
      _list[L](rd, Some(labelSelector))
    }

    private def _list[L <: ListResource[_]](
        rd: ResourceDefinition[_],
        maybeLabelSelector: Option[LabelSelector]
      ...
      if (log.isDebugEnabled) {
        val lsInfo = maybeLabelSelector map { ls =>
          s" with label selector '${ls.toString}'"
        } getOrElse ""
    ...
    def getInNamespace[O <: ObjectResource](
        name: String,
        namespace: String
    )(implicit fmt: Format[O], rd: ResourceDefinition[O], lc: LoggingContext = RequestLoggingContext()): Future[O] = {
    ...
    private[api] def _get[O <: ObjectResource](
        name: String,
        namespace: String = namespaceName
    )(implicit fmt: Format[O], rd: ResourceDefinition[O], lc: LoggingContext): Future[O] = {
    ...
    def getPodLogSource(name: String, queryParams: Pod.LogQueryParams, namespace: Option[String] = None)(
        implicit lc: LoggingContext = RequestLoggingContext()
      ...
      val request = buildRequest(HttpMethods.GET, rd, Some(nameComponent), query, watch = false, targetNamespace)
    ...
    private[skuber] def toKubernetesResponse[T](
        response: HttpResponse
    )(implicit reader: Reads[T], lc: LoggingContext): Future[T] = {
          ...
          try {
            ...
            case ex: Exception =>
              ...
              throw new K8SException(
                Status(message = Some("Error unmarshalling resource from response"), details = Some(ex.getMessage))
              )
          }
      }
    }
    ...
    private[skuber] def checkResponseStatus(
        response: HttpResponse
    )(implicit lc: LoggingContext): Future[Option[Status]] = {
  ...
  def init(k8sContext: Context, logConfig: LoggingConfig, closeHook: Option[() => Unit], appConfig: Config)(
    ...
    def dispatcherFromConfig(configKey: String): Option[ExecutionContext] =
      if (appConfig.getString(configKey).isEmpty) {
        None
      } else {
        Some(actorSystem.dispatchers.lookup(appConfig.getString(configKey)))
      }
    implicit val dispatcher: ExecutionContext =
      getSkuberConfig("akka.dispatcher", dispatcherFromConfig, actorSystem.dispatcher)

    def durationFomConfig(configKey: String): Option[Duration] =
      Some(Duration.fromNanos(appConfig.getDuration(configKey).toNanos))
    ...
    sslContext foreach { ssl =>
      val httpsContext =
        ConnectionContext.https(ssl, None, Some(scala.collection.immutable.Seq("TLSv1.2", "TLSv1")), None, None)
