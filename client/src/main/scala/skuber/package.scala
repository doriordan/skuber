...
import skuber.api.{ Configuration, client }
...
package object skuber {
  ...
  case class ListMeta(selfLink: String = "", resourceVersion: String = "")

  case class APIVersions(kind: String, versions: List[String])
  ...
  case class ObjectReference(
    ...
    def \(addPath: String): ObjectReference = this.copy(fieldPath = fieldPath + "/" + addPath)
  }
  ...
  case class HTTPGetAction(port: NameablePort, host: String = "", path: String = "", schema: String = "HTTP")
      extends Handler {
    def url: URL = {
      this.port match {
  ...
  val K8SCluster: client.Cluster.type = skuber.api.client.Cluster
  val K8SContext: client.Context.type = skuber.api.client.Context
  ...
  /**
    ...
    */
  def k8sInit(
      config: skuber.api.Configuration
