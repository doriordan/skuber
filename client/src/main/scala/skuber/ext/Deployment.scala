...
case class Deployment(
  ...
  def getPodSpec: Option[Pod.Spec] =
    for {
      spec <- this.spec
  ...
  def updateContainer(newContainer: Container): Deployment = {
    ...
    val updatedTemplate: Pod.Template.Spec =
      copySpec.template.getOrElse(Pod.Template.Spec()).copy(spec = Some(newPodSpec))
...
object Deployment {
  ...
  implicit val deployListDef: ResourceDefinition[ListResource[Deployment]] =
    new ResourceDefinition[ListResource[Deployment]] { def spec: NonCoreResourceSpecification = specification }
  ...
  object Strategy {
    private[skuber] case class StrategyImpl(_type: StrategyType.StrategyType, rollingUpdate: Option[RollingUpdate])
        extends Strategy
    def apply: Strategy = StrategyImpl(_type = StrategyType.Recreate, rollingUpdate = None)
    def apply(_type: StrategyType.StrategyType, rollingUpdate: Option[RollingUpdate]): Strategy =
      StrategyImpl(_type, rollingUpdate)
    def apply(rollingUpdate: RollingUpdate): Strategy =
      StrategyImpl(_type = StrategyType.RollingUpdate, rollingUpdate = Some(rollingUpdate))
  ...
  case class RollingUpdate(maxUnavailable: IntOrString = Left(1), maxSurge: IntOrString = Left(1))
