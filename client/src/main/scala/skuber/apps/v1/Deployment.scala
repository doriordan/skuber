...
case class Deployment(
  ...
  lazy val copySpec: Deployment.Spec =
    this.spec.getOrElse(Deployment.Spec(selector = LabelSelector(), template = Pod.Template.Spec()))
...
object Deployment {
  ...
  implicit val scDef: Scale.SubresourceSpec[Deployment] = new Scale.SubresourceSpec[Deployment] {
    override def apiVersion: Phase = appsAPIVersion
  }
  ...
  object Strategy {
    ...
    def apply(_type: StrategyType.StrategyType, rollingUpdate: Option[RollingUpdate] = None): Strategy =
  ...
  )(RollingUpdate.apply, unlift(RollingUpdate.unapply))
