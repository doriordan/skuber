...
case class StatefulSet(
  ...
  lazy val copySpec: StatefulSet.Spec = this.spec.getOrElse(StatefulSet.Spec(template = Pod.Template.Spec()))
  private val rollingUpdateStrategy =
    StatefulSet.UpdateStrategy(`type` = StatefulSet.UpdateStrategyType.RollingUpdate, None)
  private def rollingUpdateStrategy(partition: Int) =
    StatefulSet.UpdateStrategy(
      `type` = StatefulSet.UpdateStrategyType.RollingUpdate,
      Some(StatefulSet.RollingUpdateStrategy(partition))
    )

  def withReplicas(count: Int): StatefulSet = this.copy(spec = Some(copySpec.copy(replicas = Some(count))))
  def withServiceName(serviceName: String): StatefulSet =
    this.copy(spec = Some(copySpec.copy(serviceName = Some(serviceName))))
  def withTemplate(template: Pod.Template.Spec): StatefulSet =
    this.copy(spec = Some(copySpec.copy(template = template)))
  def withLabelSelector(sel: LabelSelector): StatefulSet = this.copy(spec = Some(copySpec.copy(selector = Some(sel))))
  def withRollingUpdateStrategyPartition(partition: Int): StatefulSet =
    this.copy(spec = Some(copySpec.copy(updateStrategy = Some(rollingUpdateStrategy(partition)))))
...
object StatefulSet {
  ...
  case class UpdateStrategy(
      `type`: UpdateStrategyType.UpdateStrategyType,
      rollingUpdate: Option[RollingUpdateStrategy] = None
  )
  ...
  case class Spec(
    ...
    def withVolumeClaimTemplate(claim: PersistentVolumeClaim): Spec =
      copy(volumeClaimTemplates = claim :: volumeClaimTemplates)
  }
  ...
  implicit val statefulSetPodPcyMgmtFmt: Format[StatefulSet.PodManagementPolicyType.PodManagementPolicyType] =
    Format(enumReads(StatefulSet.PodManagementPolicyType, StatefulSet.PodManagementPolicyType.OrderedReady), enumWrites)
  implicit val statefulSetRollUp: Format[StatefulSet.RollingUpdateStrategy] =
    Json.format[StatefulSet.RollingUpdateStrategy]
  ...
  )(StatefulSet.Spec.apply, unlift(StatefulSet.Spec.unapply))
  ...
  )(StatefulSet.apply, unlift(StatefulSet.unapply))
