...
package object format {
  ...
  implicit val timewWrites: Writes[Timestamp] =
    Writes.temporalWrites[ZonedDateTime, DateTimeFormatter](DateTimeFormatter.ISO_OFFSET_DATE_TIME)
  ...
  class MaybeEmpty(val path: JsPath) {
    ...
    def formatMaybeEmptyList[T](
        implicit tReads: Reads[T],
        tWrites: Writes[T],
        omitEmpty: Boolean = true
    ): OFormat[List[T]] =
      path
        .formatNullable[List[T]]
        .inmap[List[T]](_.getOrElse(emptyL[T]), l => if (omitEmpty && l.isEmpty) None else Some(l))

    def formatMaybeEmptyMap[V](
        implicit vReads: Reads[V],
        vWrites: Writes[V],
        omitEmpty: Boolean = true
    ): OFormat[Map[String, V]] =
      path
        .formatNullable[Map[String, V]]
        .inmap[Map[String, V]](_.getOrElse(emptyM[V]), m => if (omitEmpty && m.isEmpty) None else Some(m))

    def formatMaybeEmptyByteArrayMap(
        implicit vReads: Reads[Map[String, Array[Byte]]],
        vWrites: Writes[Map[String, Array[Byte]]],
        omitEmpty: Boolean = true
    ): OFormat[Map[String, Array[Byte]]] =
      path
        .formatNullable[Map[String, Array[Byte]]]
        .inmap[Map[String, Array[Byte]]](
          _.getOrElse(emptyM[Array[Byte]]),
          m => if (omitEmpty && m.isEmpty) None else Some(m)
        )
  ...
  class EnumFormatter(val path: JsPath) {
    def formatEnum[E <: Enumeration](enum: E, default: Option[E#Value] = None): OFormat[E#Value] =
      path
        ...
        .inmap[E#Value](_.map(s => enum.withName(s)).getOrElse(default.get), e => Some(e.toString))
  ...
  case class OnTheWireSelector(
      matchLabels: Option[Map[String, String]] = None,
      matchExpressions: Option[List[SelMatchExpression]] = None
  )
  ...
  class LabelSelectorFormat(path: JsPath) {
    def formatNullableLabelSelector: OFormat[Option[LabelSelector]] =
      path
        .formatNullable[OnTheWireSelector]
        .inmap[Option[LabelSelector]](
  ...
  def ListResourceFormat[O <: ObjectResource](implicit f: Format[O]): OFormat[ListResource[O]] =
    listResourceFormatBuilder[O].apply(ListResource.apply, unlift(ListResource.unapply))
    ...
    override def writes(toleration: Pod.Toleration): JsValue = toleration match {
      ...
      case Pod.EqualToleration(key, value, effect, tolerationSeconds) =>
        val fields: List[(String, JsValue)] = List(
          ...
          tolerationSeconds.map(ts => "tolerationSeconds" -> JsNumber(ts))
        ).flatten
        JsObject(fields)
  ...
  implicit val envVarValueWrite = Writes[EnvVar.Value] { value =>
    value match {
      ...
      case cmr: EnvVar.ConfigMapKeyRef =>
        (JsPath \ "valueFrom" \ "configMapKeyRef").write[EnvVar.ConfigMapKeyRef].writes(cmr)
  ...
  implicit val configMapEnvSourceFmt: Format[EnvFromSource.ConfigMapEnvSource] =
    Json.format[EnvFromSource.ConfigMapEnvSource]
  ...
  implicit val envSourceWrite = Writes[EnvFromSource.EnvSource] { value =>
    value match {
      case c: EnvFromSource.ConfigMapEnvSource =>
        (JsPath \ "configMapRef").write[EnvFromSource.ConfigMapEnvSource].writes(c)
  ...
  implicit val cntrStateWrites: Writes[Container.State] = Writes[Container.State] { state =>
    state match {
      ...
      case te: Container.Terminated =>
        (JsPath \ "terminated").write[Container.Terminated](cntrStateTerminatedFormat).writes(te)
    }
  }
  ...
  implicit val intOrStrWrites = Writes[IntOrString] { value =>
    value match {
  ...
  implicit val tcpSocketActionFormat: Format[TCPSocketAction] = (
    (JsPath \ "port")
      .format[NameablePort]
      .inmap(
        port => TCPSocketAction(port),
        (tsa: TCPSocketAction) => tsa.port
      )
    )
  ...
  implicit val handlerWrites: Writes[Handler] = Writes[Handler] { handler =>
    handler match {
  ...
  implicit val emptyDirWrites: Writes[EmptyDir] = Writes[EmptyDir] { ed =>
    ed.medium match {
  ...
  )(ObjectFieldSelector.apply, unlift(ObjectFieldSelector.unapply))
  ...
  )(DownwardApiVolumeSource.apply, unlift(DownwardApiVolumeSource.unapply))
  ...
  )(GCEPersistentDisk.apply, unlift(GCEPersistentDisk.unapply))
  ...
  implicit val iscsiFormat: Format[ISCSI] = (
    (JsPath \ "targetPortal").format[String] and
      ...
      (JsPath \ "fsType").format[String] and
      (JsPath \ "readOnly").formatMaybeEmptyBoolean()
  )(ISCSI.apply, unlift(ISCSI.unapply))
  ...
  implicit val persVolumeSourceWrites: Writes[PersistentSource] = Writes[PersistentSource] {
    ...
    case awse: AWSElasticBlockStore =>
      (JsPath \ "awsElasticBlockStore").write[AWSElasticBlockStore](awsFormat).writes(awse)
  ...
  implicit val volumeSourceWrites: Writes[Source] = Writes[Source] { source =>
    source match {
      ...
      case secr: skuber.Volume.Secret =>
        (JsPath \ "secret").write[skuber.Volume.Secret](volumeSecretFormat).writes(secr)
      case cfgMp: ConfigMapVolumeSource =>
        (JsPath \ "configMap").write[ConfigMapVolumeSource](configMapVolFormat).writes(cfgMp)
      case gitr: GitRepo => (JsPath \ "gitRepo").write[GitRepo](gitFormat).writes(gitr)
      case da: DownwardApiVolumeSource =>
        (JsPath \ "downwardAPI").write[DownwardApiVolumeSource](downwardApiVolumeSourceFormat).writes(da)
      case pvc: Volume.PersistentVolumeClaimRef =>
        (JsPath \ "persistentVolumeClaim")
          .write[Volume.PersistentVolumeClaimRef](persistentVolumeClaimRefFormat)
          .writes(pvc)
    }
  }
  ...
  implicit val nodeAffinityOperatorFormat: Format[Pod.Affinity.NodeSelectorOperator.Operator] =
    Format(enumReads(Pod.Affinity.NodeSelectorOperator), enumWrites)
  ...
  )(Pod.Affinity.MatchExpression.apply, unlift(Pod.Affinity.MatchExpression.unapply))

  implicit val nodeSelectorTermFormat: Format[Pod.Affinity.NodeSelectorTerm] = (
    (JsPath \ "matchExpressions")
      .format[Pod.Affinity.MatchExpressions]
      .inmap(
        matchExpressions => Pod.Affinity.NodeSelectorTerm(matchExpressions),
        (nst: Pod.Affinity.NodeSelectorTerm) => nst.matchExpressions
      )
    )

  implicit val nodeRequiredDuringSchedulingIgnoredDuringExecutionFormat
  ...
  implicit lazy val nodePreferredSchedulingTermFormat: Format[Pod.Affinity.NodeAffinity.PreferredSchedulingTerm] =
    Json.format[Pod.Affinity.NodeAffinity.PreferredSchedulingTerm]
  ...
  implicit lazy val nodeAffinityFormat: Format[Pod.Affinity.NodeAffinity] = (
    (JsPath \ "requiredDuringSchedulingIgnoredDuringExecution")
      .formatNullable[Pod.Affinity.NodeAffinity.RequiredDuringSchedulingIgnoredDuringExecution] and
  ...
  val podSpecPartTwoFormat: OFormat[
    (
        Option[String],
        List[Pod.HostAlias],
        Option[Boolean],
  ...
  implicit val podSpecFmt: Format[Pod.Spec] = (
    ...
    {
      case (
          (conts, initConts, vols, rpol, tgps, adls, dnspol, nodesel, svcac, node, hnet, ips, aff, tol, sc),
  ...
  implicit val loadBalStatusFmt: Format[Service.LoadBalancer.Status] =
    (JsPath \ "ingress")
      .formatMaybeEmptyList[Service.LoadBalancer.Ingress]
  ...
  implicit val serviceStatusFmt: Format[Service.Status] =
    (JsPath \ "loadBalancer")
      .formatNullable[Service.LoadBalancer.Status]
  ...
  implicit val accessModeFmt: Format[PersistentVolume.AccessMode.AccessMode] =
    Format(enumReads(PersistentVolume.AccessMode), enumWrites)
  implicit val pvolPhaseFmt: Format[PersistentVolume.Phase.Phase] =
    Format(enumReads(PersistentVolume.Phase), enumWrites)
  implicit val reclaimPolicyFmt: Format[PersistentVolume.ReclaimPolicy.ReclaimPolicy] =
    Format(enumReads(PersistentVolume.ReclaimPolicy), enumWrites)
  ...
  implicit val base64Format: Format[Array[Byte]] = (
    JsPath
      ...
      .inmap(
        ...
        (bytes: Array[Byte]) => Base64.encodeBase64String(bytes)
      )
    )
  ...
  implicit val mapStringByteArrayFormat: Format[Map[String, Array[Byte]]] =
    JsPath
      ...
      .inmap(
        _.map({ case (k, v) => k -> Base64.decodeBase64(v.getBytes) }),
  ...
  )(skuber.Secret.apply, unlift(skuber.Secret.unapply))
  ...
  implicit val limitRangeSpecFmt: Format[LimitRange.Spec] =
    (JsPath \ "items")
      .formatMaybeEmptyList[LimitRange.Item]
      .inmap(items => LimitRange.Spec(items), (spec: LimitRange.Spec) => spec.items)
  ...
  implicit val resourceQuotaSpecFmt: Format[Resource.Quota.Spec] =
    (JsPath \ "hard")
      .formatMaybeEmptyMap[Resource.Quantity]
      .inmap(hard => Resource.Quota.Spec(hard), (spec: Resource.Quota.Spec) => spec.hard)
