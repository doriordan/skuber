...
case class Container(
  ...
  def withWorkingDir(wd: String): Container = this.copy(workingDir = Some(wd))
  def withArgs(arg: String*): Container = this.copy(args = arg.toList)
  def withEntrypoint(cmd: String*): Container = this.copy(command = cmd.toList)

  def withTerminationMessagePath(path: String): Container = this.copy(terminationMessagePath = Some(path))
  ...
  def limitCPU(cpu: Resource.Quantity): Container = addResourceLimit(Resource.cpu, cpu)
  def limitMemory(mem: Resource.Quantity): Container = addResourceLimit(Resource.memory, mem)
  ...
  def requestCPU(cpu: Resource.Quantity): Container = addResourceRequest(Resource.cpu, cpu)
  def requestMemory(mem: Resource.Quantity): Container = addResourceRequest(Resource.memory, mem)
