...
case class HorizontalPodAutoscaler(
  ...
  def withResourceVersion(version: String): HorizontalPodAutoscaler =
...
object HorizontalPodAutoscaler {
  ...
  implicit val hpasListDef: ResourceDefinition[HorizontalPodAutoscalerList] =
    new ResourceDefinition[HorizontalPodAutoscalerList] { def spec: NonCoreResourceSpecification = specification }
  ...
  def build(name: String, namespace: String, kind: String, apiVersion: String = "v1"): HorizontalPodAutoscaler = {
    ...
    val scaleTargetRef = CrossVersionObjectReference(kind = kind, name = name, apiVersion = apiVersion)
